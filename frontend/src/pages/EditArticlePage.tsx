...
export function EditArticlePage() {
    ...
    if (isFetching) {
        return (
            <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-slate-400 font-medium">Chargement de l'article...</p>
            </div>
        );
    }
    ...
    if (!article && !isFetching) {
        return (
            <div className="h-[60vh] flex flex-col items-center justify-center gap-4 text-center">
                ...
                <h2 className="text-xl font-bold text-slate-800">Article introuvable</h2>
                <p className="text-slate-400 max-w-xs">Cet article n'existe pas ou a été supprimé.</p>
                <button
    ...
    return (
        <div className="max-w-[1600px] mx-auto">
            <ArticlePreviewModal
                article={isPreviewOpen ? { ...article!, ...formData, categories: categories.filter(c => formData.categoryIds.includes(c.id)), network: networks.find(n => n.id === formData.networkId) || article!.network } as any : null}
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        <button
                            type="button"
                            onClick={() => navigate('/articles')}
                            className="p-2.5 rounded-xl border border-slate-100 bg-white text-slate-400 hover:text-slate-600 hover:border-slate-200 transition-all shadow-sm"
                        >
                            <ArrowLeft className="w-5 h-5" />
                        </button>
                        <div>
                            <h1 className="text-3xl font-bold text-slate-900 tracking-tight font-serif">Modifier l'article</h1>
                            <p className="text-slate-400 font-medium mt-1">Édition de : {article?.title}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        <button
                            ...
                            className="p-2.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm hover:bg-white transition-all shadow-sm flex items-center gap-2"
                        >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div className="space-y-4 sticky top-28 h-fit">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Aperçu en temps réel</span>
                        </div>
